...
export function Step({
  ...
  children,
  ...props
}: Omit<ComponentProps<"li">, "title"> & { title?: ReactNode }) {
  return (
    <li
      className={cn(
        "relative pb-8 pl-10 last:pb-0 [counter-increment:prose-step]",
        ...
        "before:items-center before:justify-end before:text-2xs before:font-medium",
        "before:tabular-nums before:text-(color:--tone-hue)",
        "before:[content:counter(prose-step)]",
        ...
        "after:absolute after:inset-y-0 after:left-7 after:w-px after:bg-border",
        "last:after:hidden",
        className,
      )}
      {...props}
    >
      {title ? (
        <TypographyLabel as="div" className="mb-1">
          {title}
        </TypographyLabel>
      ) : null}
      <TypographyMuted as="div">{children}</TypographyMuted>
    </li>
  );
}
