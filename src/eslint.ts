...
const COLOUR_UTIL =
  "text|bg|border|ring|from|to|via|fill|stroke|divide|outline|placeholder|shadow|decoration|accent|caret";
...
function colourRules({
  ...
  return [
    ...rule(
      `/(^| )${VARIANTS}(${COLOUR_UTIL})-${PALETTE}-\\d+/`,
    ...
    ...rule(
      `/(^| )${VARIANTS}(${COLOUR_UTIL})-\\[#/`,
      "Hex colours bypass the token system entirely, gradient stops included. Add a token if the colour is real; use an existing one if it is not.",
    ),
      ...
      ? [
          {
            ...
            message:
              "A literal colour in a style object. Every colour rule reads classNames, so nothing else here can see this one. Use a token class, or `var(--token)` in the style object when a class cannot carry it. An OG card is the exception, since next/og resolves no custom properties.",
          },
        ]
      : []),
  ];
...
function surfaceAsInkRules(): RestrictedSyntax[] {
  return [
    ...rule(
      "/(^| )(dark:|hover:|focus:|group-hover:)*text-(muted|card|popover|input)($| |\\x2f)/",
    ...
    ...rule(
      "/(^| )(dark:|hover:|focus:|group-hover:)*text-(primary|secondary|success)($| |\\x2f)/",
...
function typographyRules({
  ...
  return [
    ...
    ...rule(
      `/(^| )${VARIANTS}text-\\[[\\d.]+(px|rem|em|pt|ch|vw|vh|vmin|vmax)\\]/`,
...
function markAlignRules(): RestrictedSyntax[] {
  return rule(
    `/(^| )${MARGIN_TOP}[^\\n]*${INLINE}|${INLINE}[^\\n]*(^| )${MARGIN_TOP}/`,
    "A vertical margin on an inline mark is a nudge that fits one rung and no other. Use `align-middle` for a mark inside a run of words, `ON_FIRST_LINE` for a mark beside a block of text, and `CAP_TRIM` on the text of a single-line row.",
  );
}
