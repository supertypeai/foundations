...
```ts
import { checkOptical } from "@supertype.ai/foundations/optical";
...
Browsers quantise ascent, descent and cap height to whole pixels before laying a
line out, and that rounding is what makes one rung out and the next flat. It also
makes the metrics load-bearing: `next/font` gives Ubuntu Sans a cap height of 693
where the browser measures 727, through `actualBoundingBoxAscent` on a canvas-drawn
H, and the wrong figure reports the page title rung as flat when it renders a pixel
out. Ascent and descent in that table are right, so cap height is the one to confirm.

Half a pixel is most of a ramp, so the verdict is a share rather than a pixel:
`tolerance` is a fraction of the cap band and defaults to 0.05, the point where a
miss stops reading as rounding. Pass 0 to see every rung. Read a result as a floor
rather than a count, because paint rounds the baseline again in the same direction,
and treat a rung that comes back as one to trim.
