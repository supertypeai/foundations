...
export function DisclosureChevron({ className }) {
    return (_jsx("svg", { "data-slot": "disclosure-icon", "aria-hidden": "true", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", className: cn(
        // The tab strip's icon size, and the mark's ink when open, for the reason
        // the active tab's icon takes it.
        "pointer-events-none size-3.5 shrink-0 text-muted-foreground", "transition-[transform,color] duration-200 ease-out motion-reduce:transition-none", "group-open/disclosure:rotate-180 group-open/disclosure:text-(color:--tone-hue)", "group-aria-expanded/disclosure-row:rotate-180 group-aria-expanded/disclosure-row:text-(color:--tone-hue)", className), children: _jsx("path", { d: "m6 9 6 6 6-6" }) }));
}
